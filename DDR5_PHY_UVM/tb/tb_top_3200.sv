/*==============================================================================
** File    : tb_top_3200.sv
** Purpose : Testbench cho DDR5-3200 Speed Test
**
** DDR5-3200 theo JEDEC:
**   Data Rate   = 3200 MT/s
**   Clock (PHY) = 1600 MHz  →  period = 0.625 ns
**
** Khác biệt duy nhất so với tb_top.sv:
**   - CLK_PERIOD = 0.000625 (ms) với timescale 1ns/1fs
**     → dùng timescale 1ps/1fs và CLK_PERIOD_PS = 625 (ps)
**   - Module name = tb_top_3200
...
module tb_top_3200;
    ...
    // DDR5-3200: clock 1600 MHz → half period = 312 ps
    logic clk;
    initial clk = 0;
    always #312 clk = ~clk;   // 312ps × 2 = 624ps ≈ 0.625ns (1603 MHz)

    // Dùng lại interface GỐC — không cần interface mới
    ...
    initial begin
        // Dùng key "vif" giống hệt tb_top.sv gốc → driver/monitor gốc hoạt động bình thường
        uvm_config_db #(virtual ddr5_phy_if)::set(null, "uvm_test_top.*", "vif", vif);
        $display("==============================================================");
        $display("  SPEED TEST : DDR5-3200");
        $display("  Clock      : ~1603 MHz  (half-period = 312 ps)");
        $display("  Data Rate  : ~3200 MT/s");
        $display("==============================================================");
    end
    ...
    initial begin
        $dumpfile("dump_3200.vcd");
        $dumpvars(0, tb_top_3200);
    end
